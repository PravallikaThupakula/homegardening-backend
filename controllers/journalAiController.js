...
export const getReflection = async (req, res) => {
  try {
    ...
    if (!reflection) {
      return res.json({
        reflection:
          "Keep nurturing your plants! Your consistency is paying off.",
      });
    }
...
/* ================= DETECT MOOD ================= */
export const detectMood = async (req, res) => {
  try {
    const { notes } = req.body || {};
    const text =
      typeof notes === "string" ? notes.trim() : "";
    ...
    const normalized = allowed.includes(mood)
      ? mood
      : "neutral";
